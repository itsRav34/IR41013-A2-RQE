...
baseLUR3 = transl([0.5,-0.1,0.7])*trotz(-pi);
lur3 = LinearUR3(baseLUR3);
lur3.model.animate(lur3.model.getpos());
PlaceObject('emergencyStopButton.ply', [0.2,0,0.7]);

%Placement of TM5-900 robot
baseTM5 = transl([0.6,-1.0,0.7])*trotz(-pi);
tm5 = TM5(baseTM5);
tm5.model.animate(tm5.model.getpos());
PlaceObject('emergencyStopButton.ply', [0.2,-1.0,0.7]);

%Placement of rack and 3 test tubes coloured Red
TTR = testTubeR();

R1 = TTR.CreateTT(0.975,-0.48,0.72, 0);
% PlaceObject('testTubeR.ply', [-0.325,-0.48,0.72]);

R2 = TTR.CreateTT(0.975,-0.55,0.72, 0);
% PlaceObject('testTubeR.ply', [-0.325,-0.55,0.72]);

R3 = TTR.CreateTT(0.975,-0.62,0.72, 0);
% PlaceObject('testTubeR.ply', [-0.325,-0.62,0.72]);
PlaceObject('testTubeRack.ply', [1,-0.3,0.62]);

%Placement of second rack and 3 test tubes coloured Blue
TTB = testTubeB();

% B1 = TTB.CreateTT(-0.125, -0.48, 0.72, 0);
PlaceObject('testTubeB.ply', [1.175,-0.48,0.72]);

% B2 = TTB.CreateTT(-0.125,-0.55,0.72, 0);
PlaceObject('testTubeB.ply', [1.175,-0.55,0.72]);

% B3 = TTB.CreateTT(-0.125,-0.62,0.72, 0);
PlaceObject('testTubeB.ply', [1.175,-0.62,0.72]);
PlaceObject('testTubeRack.ply', [1.2,-0.3,0.62]);

%Placement of third rack and 3 test tubes coloured Green
TTG = testTubeG();

% G1 = TTG.CreateTT(0.075,-0.48,0.72, 0);
PlaceObject('testTubeG.ply', [1.375,-0.48,0.72]);

% G2 = TTG.CreateTT(0.075,-0.55,0.72, 0); 
PlaceObject('testTubeG.ply', [1.375,-0.55,0.72]);

% G3 = TTG.CreateTT(0.075,-0.62,0.72, 0);
PlaceObject('testTubeG.ply', [1.375,-0.62,0.72]);
PlaceObject('testTubeRack.ply', [1.4,-0.3,0.62]);

PlaceObject('TestTubeRackWhole.ply', [0.7,-0.6,0.7]);

PlaceObject('LabTable2.ply', [1.5,0.2,0]);
