classdef ObjectBaseClass < handle
    %% ObjectBaseClass The base Object class from which other UTS Object models should inherit 
    ...
    properties
        ...
        % Name of the Object (to be copied into model.name)
        name;
...
%% General class for multiDOF Object simulation
        function self = ObjectBaseClass()
            ...
            try 
                ...
                self.name = ['ObjectBaseClass',datestr(now,'yyyymmddTHHMMSSFFF')];
                warning(['Please include a variable called plyFileNameStem in your inherreting class. For now the Object is named: ',self.name])                
            end
            
        end

%% delete
        % Try to clean up gracefully by deleting just this Object plot
        function delete(self)
            handles = findobj('Tag', self.model.name);
            h = get(handles,'UserData');
            try delete(h.Object); end 
...
%% PlotAndColourObject
        % Given a Object index, add the glyphs (vertices and faces) and colour them in if data is available 
        function PlotAndColourObject(self)
            ...
            if exist([self.plyFileNameStem,'.mat'],'file') == 2 && exist([self.plyFileNameStem,'Link0.ply'],'file') ~= 2
                warning('There are no ply files for the links but there is a mat file. You should use PlotAndColourObjectMat to create and colour a 3D Object model plot. I am doing this for you now.')
                self.PlotAndColourObjectMat()
                return;
            end
            ...
            h = self.InitiliseObjectPlot();
            if 1 < length(h)
                self.MultiObjectWarningMessage();
                h = h{1};
            end
...
% containing data for the vertex, face and colour data in it 
        function PlotAndColourObjectMat(self)
    ...
    methods (Hidden)
%% InitiliseObjectPlot
% First and only time to plot the Object
        function h = InitiliseObjectPlot(self)
            self.figure_h = gcf;
            self.axis_h = gca;
            ...
            self.workspace = [-3, 3, -3, 3, 0, 1]; %[-roughMinMax roughMinMax -roughMinMax roughMinMax -0.01 roughMinMax]; 

            self.model.plot3d(self.homeQ,'noarrow','workspace',self.workspace,'view',[-8,10], 'noarrow', 'nowrist');%,'notiles');            
...
%% MoveBaseToTR 
        % move Object base through a ctraj generated path
...
%% MultiObjectWarningMessage
        function MultiObjectWarningMessage(self)
            disp('There are more than 1 of these Objects in the plot although they may be plotted over each other. This shouldnt break anything, but it is a good idea to clean up by deleting identical Objects before you replot a new one. For now I will recolour the first (and probably latest) one.')
        end
    end
end